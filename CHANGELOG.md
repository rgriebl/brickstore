...
A few fixes and improvements, plus some new features:
- Improved both the quality and performance of the 3D part renderer.
...
- Both Snaps and Flatpaks are now available for ARM processors as well.
- Added a Windows Universal installer (Intel and ARM combined).
- BrickStore is now also published to the MicroSoft store. This helps with installations where Windows will not let you install the unsigned installer from GitHub.
