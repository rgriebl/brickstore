...
void MultiProgressBar::languageChange()
{
...
void MultiProgressBar::setStopIcon(const QIcon &ico)
{
    m_stop->setIcon(ico);
}

void MultiProgressBar::resizeEvent(QResizeEvent *)
{
    int h = height();
    int w = width();
    bool too_small = (w < h);
...
MultiProgressBar::ItemData::ItemData(const QString &label)
{
    m_label = label;
    m_progress = 0;
    m_total = 0;
...
int MultiProgressBar::addItem(const QString &label, int id)
{
    if (id < 0)
        id = --m_autoid;
...
void MultiProgressBar::removeItem(int id)
{
...
QString MultiProgressBar::itemLabel(int id) const
{
...
int MultiProgressBar::itemProgress(int id) const
{
    QMap<int, ItemData>::const_iterator it = m_items.find(id);
...
int MultiProgressBar::itemTotalSteps(int id) const
{
    QMap<int, ItemData>::const_iterator it = m_items.find(id);
...
void MultiProgressBar::setItemProgress(int id, int progress)
{
    QMap<int, ItemData>::iterator it = m_items.find(id);
...
void MultiProgressBar::setItemProgress(int id, int progress, int total)
{
    QMap<int, ItemData>::iterator it = m_items.find(id);
...
void MultiProgressBar::setItemTotalSteps(int id, int total)
{
    QMap<int, ItemData>::iterator it = m_items.find(id);
...
void MultiProgressBar::setItemLabel(int id, const QString &label)
{
    QMap<int, ItemData>::iterator it = m_items.find(id);
