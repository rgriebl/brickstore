...
class TaskPaneManagerPrivate {
public:
    TaskPaneManager::Mode  m_mode;
...
public:
    TaskPane(TaskPaneManager *manager, TaskPaneManagerPrivate *priv);
...
private:
    TaskPaneManager        *q;
    TaskPaneManagerPrivate *d;
};

TaskPane::TaskPane(TaskPaneManager *manager, TaskPaneManagerPrivate *priv)
...
void TaskPane::animateExpand(qreal value)
{
...
void TaskPane::changeEvent(QEvent *e)
{
...
void TaskPane::resizeEvent(QResizeEvent *e)
{
    updateBackground();
    update(recalcLayout());
...
void TaskPane::updateBackground()
{
#if defined( Q_WS_WIN )
...
int TaskPane::findItemHeader(const QPoint &p)
{
    int idx = 0;
    foreach (const TaskPaneManagerPrivate::Item &item, d->m_items) {
...
void TaskPane::mouseMoveEvent(QMouseEvent *e)
{
...
void TaskPane::leaveEvent(QEvent *e)
{
    if (d->m_hot_item > -1) {
        unsetCursor();
        update(d->m_items[d->m_hot_item].m_header_rect);
...
void TaskPane::mouseReleaseEvent(QMouseEvent *e)
{
    int idx = findItemHeader(e->pos());
...
void TaskPane::paintEvent(QPaintEvent *e)
{
    QPainter p(this);
...
QSize TaskPane::sizeHint() const
{
    if (!d->m_size_hint.isValid())
        const_cast<TaskPane *>(this)->recalcLayout();
    return d->m_size_hint;
}

QRegion TaskPane::recalcLayout()
{
    //updateGeometry();
...
TaskPaneManager::TaskPaneManager(QMainWindow *parent)
        : QObject(parent)
{
    d = new TaskPaneManagerPrivate;
...
int TaskPaneManagerPrivate::findItem(QWidget *w)
{
    int idx = 0;
...
void TaskPaneManager::addItem(QWidget *w, const QIcon &is, const QString &text)
{
    if (w && (d->findItem(w) == -1)) {
        TaskPaneManagerPrivate::Item item;
        item.m_itemdock   = 0;
        item.m_widget     = w;
        item.m_icon       = is;
        item.m_text       = text;
...
void TaskPaneManager::removeItem(QWidget *w, bool delete_widget)
{
...
void TaskPaneManager::setItemText(QWidget *w, const QString &txt)
{
    int idx = d->findItem(w);
...
QString TaskPaneManager::itemText(QWidget *w) const
{
    int idx = d->findItem(w);
...
bool TaskPaneManager::isItemVisible(QWidget *w) const
{
    int idx = d->findItem(w);
    return idx == -1 ? false : d->m_items[idx].m_visible;
}

void TaskPaneManager::setItemVisible(QWidget *w, bool visible)
{
    int idx = d->findItem(w);
...
TaskPaneManager::Mode TaskPaneManager::mode() const
{
    return d->m_mode;
}

void TaskPaneManager::setMode(Mode m)
{
    if (m == d->m_mode)
        return;
...
void TaskPaneManager::create()
{
    ...
    if (d->m_mode == Modern) {
        ...
        d->m_taskpane = new TaskPane(this, d);
...
void TaskPaneManager::dockVisibilityChanged(bool b)
{
...
QMenu *TaskPaneManager::createItemVisibilityMenu() const
{
...
void TaskPaneManager::itemMenuAboutToShow()
{
...
void TaskPaneManager::itemMenuTriggered(QAction *a)
{
    if (!a)
        return;
    ...
    if (id == -4) {
        ...
        int index = 0;
        for (QList<TaskPaneManagerPrivate::Item>::iterator it = d->m_items.begin(); it != d->m_items.end(); ++it, index++) {
            TaskPaneManagerPrivate::Item &item = *it;
...
public:
    TaskPaneAction(TaskPaneManager *tpm, QObject *parent, const char *name = 0)
        : QAction(parent)
    {
        setObjectName(name);
...
QAction *TaskPaneManager::createItemVisibilityAction(QObject *parent, const char *name) const
{
    return new TaskPaneAction(const_cast<TaskPaneManager *>(this), parent, name);
}
