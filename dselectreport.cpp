...
public:
    ReportModel()
    {
        ...
        m_reports = CReportManager::inst()->reports();
    }
...
public:
    QModelIndex index(int row, int column, const QModelIndex &parent) const
    {
        if (hasIndex(row, column, parent))
            return parent.isValid() ? QModelIndex() : createIndex(row, column, const_cast<CReport *>(m_reports.at(row)));
        return QModelIndex();
    }
                
    const CReport *report(const QModelIndex &index) const
    {
        return index.isValid() ? static_cast<const CReport *>(index.internalPointer()) : 0;
    }
                    
    QModelIndex index(CReport *report) const
    {
...
DSelectReport::DSelectReport(QWidget *parent, Qt::WindowFlags f)
	: QDialog(parent, f)
{
    setupUi(this);
...
void DSelectReport::reportChanged()
{
    w_buttons->button(QDialogButtonBox::Ok)->setEnabled((report()));
}

void DSelectReport::reportConfirmed()
{
	reportChanged();
	w_buttons->button(QDialogButtonBox::Ok)->animateClick();
}

const CReport *DSelectReport::report() const
{
