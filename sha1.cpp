...
{
   return calc ( data. data ( ), data. count ( ));
}
...
QByteArray calc ( const char *data, unsigned int len )
{
   ...
   SHA1Context ctx;
   ok &= ( SHA1Reset ( &ctx ) == shaSuccess );
   ok &= ( SHA1Input ( &ctx, (uint8_t *) data, len ) == shaSuccess );
   ok &= ( SHA1Result ( &ctx, (uint8_t *) digest. data ( )) == shaSuccess );

   return ok ? digest : QByteArray ( );
}
