...
QString CApplication::systemDataDir() const
{
    QString system;

#if defined( Q_WS_X11 )
#if defined( SYSTEM_DATA_DIR )
    system = QLatin1String( SYSTEM_DATA_DIR );
#else
    // Linux FHS - need something a bit more cross platform though!
    system = QLatin1String("/usr/share/brickstore/");
#endif

#elif defined( Q_WS_MACX )
    // Data dir within the bundle
    system =  qApp->applicationDirPath() + "/../Data/";

#elif defined( Q_WS_WIN )
#if !defined( Q_DEBUG )
    // app directory
    system = qApp->applicationDirPath() + "/";
#else
...
QString CApplication::userDataDir() const
{
    QString user;

#if defined( Q_WS_X11 )
    user = QDir::homePath() + "/.brickstore/";

#elif defined( Q_WS_MACX )
    user = QDir::homePath() + "/Library/BrickStore/";

#elif defined( Q_WS_WIN )
    const char *appdata = ::getenv("APPDATA");

    if (appdata && appdata[0])
        user = QLatin1String(appdata) + "/BrickStore";
